...
describe("main", () => {
  ...
  it("converts markdown to PDF successfully", async () => {
    ...
    expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining("PDF saved to"));
  });
  ...
  it("exits with error when PDF generation fails", async () => {
    ...
    expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining("PDF generation failed"));
    expect(mockExit).toHaveBeenCalledWith(1);
  });
  ...
  it("displays correct file size in output", async () => {
    ...
    expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining("5.00 MB"));
  });
  ...
  it("extracts error message when mdToPdf throws an Error", async () => {
    ...
    expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining("Puppeteer crashed"));
    expect(mockExit).toHaveBeenCalledWith(1);
  });
